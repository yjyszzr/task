...
public class DlPrintLotteryService {
		...
		if ("0".equals(retCode)) {
			...
			for (BackQueryStake stake : queryStakes) {
				...
				if (null != lotteryPrint) {
					...
					String sp = stake.getSp();
					// String comparePrintSp = getComparePrintSpHenan(sp,
					// stake.getTicketId());
					// comparePrintSp =
					// StringUtils.isBlank(comparePrintSp)?sp:comparePrintSp;

					// String game = lotteryPrint.getGame();
					// String printSp = null;
					// if("T51".equals(game) &&
					// StringUtils.isNotBlank(comparePrintSp)) {
	...
	private static void queryOrder(String heNanUrl, String heNanMerchant, String heNanPwd) {
		// String[] orders=new
		// String[]{"2018070220336551340874","2018070220343831340875","2018070220344721290876","2018070220342271280877"};
		// 2018070220336551340874
		// 2018070220343831340875
		// 2018070220344721290876
		// 2018070220342271280877
		// String[] orders=new
		// String[]{"2018070220344071350880","2018070220343531290879","2018070220347661330878"};
	 ...
	 * @return
	 */
	private List<LotteryPrintDTO> getDaLeTouLotteryPrintDTO(DlJcZqMatchBetParam param) {
		List<LotteryPrintDTO> lotteryPrints = new ArrayList<LotteryPrintDTO>();
		String playType = param.getPlayType();
		int times = param.getTimes();
	...
	private Map<String, List<MatchBetPlayCellDTO>> getMatchBetPlayMap(List<MatchBetPlayDTO> matchBellCellList) {
		// 整理投注对象
		// TMatchBetInfoWithMinOddsList tbml = new
		// TMatchBetInfoWithMinOddsList();
		// List<Double> minList = new
		// ArrayList<Double>(matchBellCellList.size());
		Map<String, List<MatchBetPlayCellDTO>> playCellMap = new HashMap<String, List<MatchBetPlayCellDTO>>(matchBellCellList.size());
		// Map<String, Double> minCellOddsMap = new HashMap<String,
		// Double>(matchBellCellList.size());
		// Map<String, Integer> cellNumsMap = new HashMap<String,
		// Integer>(matchBellCellList.size());
		matchBellCellList.forEach(betPlayDto -> {
			...
			// Integer cellNums = cellNumsMap.get(playCode);
				...
				for (MatchBetCellDTO cell : matchBetCells) {
					...
					// logger.info("=====cell.getFixedOdds()============  " +
					// cell.getFixedOdds());
					playCellDto.setFixedodds(cell.getFixedOdds());
					list.add(playCellDto);
					/*
					 * if(betCells.size() == 1) { String cellOdds =
					 * betCells.get(0).getCellOdds(); minCellOdds =
					 * Double.min(minCellOdds, Double.valueOf(cellOdds)); }else
					 * { String cellOdds =
					 * betCells.stream().min((item1,item2)->Double
					 * .valueOf(item1.
					 * getCellOdds()).compareTo(Double.valueOf(item2
					 * .getCellOdds()))).get().getCellOdds(); minCellOdds =
					 * Double.min(minCellOdds, Double.valueOf(cellOdds)); }
					 */
				}
		 ...
		 * tbml.setMinOddsList(minList); tbml.setPlayCellMap(playCellMap);
		 */
		...
		for (String issueAndGame : issueAndGameList) {
			...
			if (dto != null && StringUtils.isNotEmpty(dto.getUrl())) {
				...
				try {
					...
					for (DlPrintLottery updateDlPrint : dlPrintList) {// FIXME
																		// 后面有时间改为批量更新
			...
			try {
				...
				while (!CollectionUtils.isEmpty(lotteryLists)) {
					...
					if (stakeResponseDto == null) {
						...
						log.error("出票失败，channelId={},channelName={},errorCode={},errorMsg={}", printComEnums.getPrintChannelId(), printComEnums.getPrintChannelName(), stakeResponseDto.getRetCode(), stakeResponseDto.getRetDesc());
					} else if (stakeResponseDto.getRetSucc()) {
			...
			try {
				...
				while (!CollectionUtils.isEmpty(lotteryLists)) {
					...
					if (queryStakeResponseDTO == null) {
						...
						log.error("出票查询失败，channelId={},channelName={},errorCode={},errorMsg={}", printComEnums.getPrintChannelId(), printComEnums.getPrintChannelName(), queryStakeResponseDTO.getRetCode(), queryStakeResponseDTO.getRetDesc());
					} else if (queryStakeResponseDTO.getQuerySuccess()) {
				...
				if (Integer.valueOf(1).equals(printComEnums.getRewardType())) {// 按票计算奖金
					while (!CollectionUtils.isEmpty(lotteryLists)) {
						...
						if (!queryRewardResponseDTO.getQuerySuccess()) {
							log.error("出票查询第三方奖金失败，channelId={},channelName={},errorCode={},errorMsg={}", printComEnums.getPrintChannelId(), printComEnums.getPrintChannelName(), queryRewardResponseDTO.getRetCode(), queryRewardResponseDTO.getRetDesc());
					...
					for (String issue : issueAndGameList) {
						...
						if (!queryRewardResponseDTO.getQuerySuccess()) {
							log.error("出票查询失败，channelId={},channelName={},errorCode={},errorMsg={}", printComEnums.getPrintChannelId(), printComEnums.getPrintChannelName(), queryRewardResponseDTO.getRetCode(), queryRewardResponseDTO.getRetDesc());
						} else if (queryRewardResponseDTO.getQuerySuccess()) {
							updateLotterysReward(queryRewardResponseDTO, ticketIdPrintLottery);
						}
					}
